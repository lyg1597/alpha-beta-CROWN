...
class EpsModel(torch.nn.Module):
    ...
    def forward(self, x:torch.Tensor, A0inv: torch.Tensor):
        t2 = x@A0inv
        t4 = t2@t2 
        t8 = t4@t4
        t16 = t8@t8
...
class BoundInverse(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x, *args, **kwargs):
        ...
        #     [T/np.sqrt(2), T]
        # ]]).to(A0.device)
        val = torch.norm(delta@A0inv, dim=(1,2))   
        T = torch.norm(A0inv, dim=(1,2))*val**16/(1-val)
        mask = torch.Tensor([
            [1, 1/np.sqrt(2)],
            [1/np.sqrt(2), 1]
        ]).to(A0.device)
        T_mat = T[:,None,None]*mask[None]
        ...
        #     lb_eps, ub_eps, A_eps = model_eps_bounded.compute_bounds(
        #         x=(my_input, A0inv), method='alpha-crown', return_A=True, needed_A_dict=required_A,
        #     )
        ...
        if last_lA is None:
            ...
            positive_lA = last_lA_view.clamp(min=0)  # Keeps positive values, zeros the rest
            negative_lA = last_lA_view.clamp(max=0)  # Keeps negative values, zeros the rest
            term1 = torch.einsum('ijk,jkab->ijab', positive_lA, EpsAlb_const)
            term2 = torch.einsum('ijk,jkab->ijab', negative_lA, EpsAub_const)
            lA = term1 + term2

            lbias = -torch.einsum("xyuv,yuv->xy", lA, A0_const)          
            
            term1 = torch.einsum('ixa,xa->ix', positive_lA, Epsbiaslb_const)
            term2 = torch.einsum('ixa,xa->ix', negative_lA, Epsbiasub_const)
            lbias = lbias+term1+term2
        ...
        if last_uA is None:
            ...
            # lA = torch.einsum('ijk,jkab->ijab', last_lA_view, EpsAlb_const)
            positive_uA = last_uA_view.clamp(min=0)  # Keeps positive values, zeros the rest
            negative_uA = last_uA_view.clamp(max=0)  # Keeps negative values, zeros the rest
            term1 = torch.einsum('ijk,jkab->ijab', positive_uA, EpsAub_const)
            term2 = torch.einsum('ijk,jkab->ijab', negative_uA, EpsAlb_const)
            uA = term1 + term2

            ubias = -torch.einsum("xyuv,yuv->xy", uA, A0_const)

            term1 = torch.einsum('ixa,xa->ix', positive_uA, Epsbiasub_const)
            term2 = torch.einsum('ixa,xa->ix', negative_uA, Epsbiaslb_const)
            ubias = lbias+term1+term2
