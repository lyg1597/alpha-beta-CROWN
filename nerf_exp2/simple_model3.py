...
class AlphaModel(torch.nn.Module):
    ...
    def prepare_render_coefficients(self, tile_coord):
        if tile_coord is None:
            ...
            self.tile_coord = tile_coord[None,:,None,:].to(self.device)
    ...
    def forward(self, x):
        ...
        #     J12 * J02 * cov_cam[:,:,2, 2]
        # )
        cov2D00 = cov2D00+0.0001 
        cov2D11 = cov2D11+0.0001
...
class DepthModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        depth = means_cam[:,:,2]
        depth_diff = depth[:,:,None] - depth[:,None,:]
        return torch.heaviside(depth_diff, torch.zeros(depth_diff.shape).to(depth_diff.device))
