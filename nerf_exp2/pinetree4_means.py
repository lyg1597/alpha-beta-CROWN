...
def computeT_new_new(
    ...
    for i in range(0,N,batch_size):  
        ...
        diff = overall_depth[:,:,None] - overall_depth[:,None,i:i+batch_size]
        ...
        diff[:, batch_indices, offset_indices] = 0  # j=k masking
        

        step = torch.heaviside(diff, torch.zeros(diff.shape, device=diff.device))
        # step_U = torch.heaviside(diff, torch.zeros(diff.shape))

        term_lb = 1-step*A_ub[:,None, i:i+batch_size]                         # P*N
        term_ub = 1-step*A_lb[:,None, i:i+batch_size]                         # P*N
...
def get_rect_set(
    ...
    inp_mean = torch.clone(x).repeat(means.shape[0],1,1)
    ...
    model_mean_bounded = BoundedModule(model_mean, (means_hom_tmp, inp_mean), device = means.device)
    x_L = means_hom_tmp.clone()
    x_L[mask_trees,1,:] = x_L[mask_trees,1,:]+means_eps_L
    x_U = means_hom_tmp.clone()
    x_U[mask_trees,1,:] = x_U[mask_trees,1,:]+means_eps_U
    ptb_mean = PerturbationLpNorm(norm=np.inf, x_L=x_L, x_U=x_U)
    means_hom_tmp_ptb = BoundedTensor(means_hom_tmp, ptb_mean)
    lb_mean, ub_mean = model_mean_bounded.compute_bounds(x=(means_hom_tmp_ptb, inp_mean), method='ibp')
    ...
    radii = model_mean.get_radii(means_hom_tmp, inp_mean)
...
def compute_tile_color(
    ...
    overall_depth = torch.zeros((1,0)).to(means_strip.device)
    ...
    for j in range(0, N, gauss_step):
        ...
        mask_trees_strip_bs = mask_trees_strip[j:j+gauss_step]
        ...
        model_alpha_bounded = BoundedModule(
            model_alpha, 
            (means_hom_tmp, inp_alpha, cov_world, opacities_rast), 
        ...
        x_L = means_hom_tmp.clone() 
        x_L[mask_trees_strip_bs,1,:] = x_L[mask_trees_strip_bs,1,:]+eps_lb
        x_U = means_hom_tmp.clone() 
        x_U[mask_trees_strip_bs,1,:] = x_U[mask_trees_strip_bs,1,:]+eps_ub
        ptb_alpha = PerturbationLpNorm(norm=np.inf, x_L=x_L, x_U=x_U)
        # print(">>>>>> Starting BoundedTensor")
        inp_alpha_means = BoundedTensor(means_hom_tmp, ptb_alpha)
        # prediction = model_alpha_bounded(inp_alpha)
        tmp = time.time()
        tmp_lb_alpha, tmp_ub_alpha = model_alpha_bounded.compute_bounds(
            x=(inp_alpha_means, inp_alpha, cov_world, opacities_rast), 
            method='crown'
        )
        ...
        # print(">>>>>> Starting Bounded Module")
        model_depth_bounded = BoundedModule(model_depth, (means_hom_tmp,inp_depth), device=means_strip.device)
        # print(">>>>>> Starting PerturbationLpNorm")
        ...
        prediction = model_depth_bounded(means_hom_tmp, inp_depth)
        prediction = prediction.transpose(0,1)
        overall_depth = torch.cat((overall_depth, prediction), dim=1)
    ...
    # bounds_colors[0,:,mask_trees_strip,0] = (bounds_colors[0,:,mask_trees_strip,0]).clip(min=0.0)
    # bounds_colors[1,:,mask_trees_strip,0] = (bounds_colors[1,:,mask_trees_strip,0]+0.5).clip(max=1.0)
...
if __name__ == "__main__":
    ...
    # Tree1
    mask_tree1 = (means[:,0]>=0.476) & (means[:,0]<=0.508) & (means[:,1]>=-0.1) & (means[:,1]<=-0.061) & (means[:,2]>=-0.162) & (means[:,2]<=-0.06)
    # means = means[~mask]
    # quats = quats[~mask]
    # opacities = opacities[~mask]
    # scales = scales[~mask]
    ...
    # Tree2
    mask_tree2 = (means[:,0]>=0.6) & (means[:,0]<=0.64) & (means[:,1]>=-0.1) & (means[:,1]<=-0.061) & (means[:,2]>=-0.158) & (means[:,2]<=-0.055)
    # means = means[~mask]
    # quats = quats[~mask]
    # opacities = opacities[~mask]
    # scales = scales[~mask]
    ...
    eps_ub = torch.Tensor([[0,0,0,0.0,0.0,0.0]]).to(means.device)
    tile_size_global = 10
    gauss_step = 10000
    ...
    for i in range(10):
        ...
        for h in range(0, height, tile_size_global):
            for w in range(0, width, tile_size_global):
                ...
                print(f">>>>>>>> {i}, {h}, {w}, {means_strip.shape[0]}")
    ...
    res_lb_enlarged.save('pinetree4_means_lb.png')
    res_ub_enlarged.save('pinetree4_means_ub.png')
