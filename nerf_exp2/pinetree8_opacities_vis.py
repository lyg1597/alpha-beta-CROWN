...
if __name__ == "__main__":
    ...
    opacities_mask = (means[:,0]>=-0.023) & (means[:,0]<=0.087) & (means[:,1]>=-0.153) & (means[:,1]<=-0.096) & (means[:,2]>=-0.171) & (means[:,2]<=-0.06)
    opacities[opacities_mask] = -100
    ...
    #     [0, 0, 0, 1],
    # ]]).to('cuda')
    res_lb = np.zeros((width, height, 3))+1e10 
    res_ub = np.zeros((width, height, 3))-1e10
    ...
    res_lb_enlarged.save('pinetree8_emp_lb.png')
    res_ub_enlarged.save('pinetree8_emp_ub.png')
