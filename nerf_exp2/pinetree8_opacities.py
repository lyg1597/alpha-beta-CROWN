...
def compute_tile_color(
    ...
    for j in range(0, N, gauss_step):
        ...
        opacities_rast_lb = opacities_rast.clone() 
        opacities_rast_lb[opacities_mask_strip[j:j+gauss_step]] = (opacities_rast_lb[opacities_mask_strip[j:j+gauss_step]]-0.5).clip(min=0)
        lb_alpha = lb_alpha*opacities_rast_lb
    ...
    # bounds_colors[0,:,opacities_mask_strip,0] = (bounds_colors[0,:,opacities_mask_strip,0]-0.5).clip(min=0.0)
...
if __name__ == "__main__":
    ...
    scales = scales[mask]
    colors = colors[mask]
    opacities_mask = opacities_mask[mask]
    ...
    for h in range(0, height, tile_size_global):
        for w in range(0, width, tile_size_global):
            ...
            opacities_mask_strip = opacities_mask[in_mask]
