...
class BoundInverse(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x, *args, **kwargs):
        ...
        lb_eps, ub_eps, A_eps = model_eps_bounded.compute_bounds(x=(my_input, ), method='crown', return_A=True, needed_A_dict=required_A, can_skip=True)
    ...
    def interval_propagate(self, *v):
        ...
        required_A = defaultdict(set)
        required_A[model_eps_bounded.output_name[0]].add(model_eps_bounded.input_name[0])
        lb_eps, ub_eps = model_eps_bounded.compute_bounds(x=(my_input, ), method='ibp', can_skip=True) 
