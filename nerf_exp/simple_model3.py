...
class AlphaDepthModel(torch.nn.Module):
    def __init__(
        self,
        data_pack,
    ...
    def to(self, *args, **kwargs):
        ...
        super(AlphaDepthModel, self).to(*args, **kwargs)
    ...
    def forward(self, x):
        ...
        # alphac = alpha_clip*self.colors

        res = torch.cat((alpha_clip, means_cam[:,:,2:].unsqueeze(1)), dim=1)
...
class SortModel(torch.nn.Module):
    ...
    def forward(self, x, y):
        depth = x
        val = y
        sorted_val = self.sort_op(depth, val)
        return sorted_val
...
class RGBModel(torch.nn.Module):
    ...
    def forward(self, x, y):
        ...
        sorted_alphac = y     # 1*p*n*3 tensor: batch size; number of pixels; number of gaussians; 3 numbers for three channels

        sorted_T = torch.cat([torch.ones_like(sorted_alpha[:,:,:1]), 1-sorted_alpha[:,:,:-1]], dim=2).cumprod(dim=2)
        rgb_color = (sorted_T*sorted_alphac).sum(dim=2)
        return rgb_color 